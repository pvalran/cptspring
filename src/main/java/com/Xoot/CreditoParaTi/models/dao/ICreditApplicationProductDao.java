...
public interface ICreditApplicationProductDao extends CrudRepository<CreditApplicationProduct, Integer>, JpaSpecificationExecutor<CreditApplicationProduct> {
	...
	@Query(nativeQuery = true, value = "SELECT * FROM credits_aplication_products WHERE status_flag = 1")
	public List<CreditApplicationProduct> findAllActive();

	@Query(nativeQuery = true, value = "SELECT * FROM  credits_aplication_products WHERE status_flag = 1 AND name=:name LIMIT 1")
	public CreditApplicationProduct findByNameActive(@Param("name") String name);
	
	@Query(nativeQuery = true, value = "SELECT * FROM  credits_aplication_products WHERE name=:name LIMIT 1")
